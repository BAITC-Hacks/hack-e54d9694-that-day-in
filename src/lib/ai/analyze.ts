...
export async function analyzeEvidence(evidence: EvidenceFact[], options: {
  ...
  const timeout = Math.max(1, Math.min(options.timeoutMs ?? AI_TIMEOUT_MS, AI_TIMEOUT_MS));
  ...
  try {
    ...
    const result = await Promise.race([
      deadline,
      transport({ model: options.model ?? DEFAULT_MODEL, evidence, signal: controller.signal }),
    ]);
